...
void add_client(int fd) {
  ...
  clientholder[++clientholder_size] = fd;
  pthread_mutex_unlock(&clientholder_mtx);
}
...
int find_random_client(void) {
  ...
  if (clientholder_size == -1) {
    ...
    int fd = clientholder[rand() % (clientholder_size + 1)];
    pthread_mutex_unlock(&clientholder_mtx);
    return fd;
  }
}
...
void* ping_clients(void* args){
  while(1){
    ...
    int client = find_random_client();
    if(client != -1){
      if(send(client, &msg, sizeof(msg), 0) == -1){
        perror("ping sending failure");
        close_client(client);
      }
    }
    sleep(3);
...
void *networking_function(void *args){
  ...
  while(1){
    ...
    for(int i = 0; i < n; i++){
      ...
      else if(event.data.fd == inet_socket || event.data.fd == unix_socket){
          while(1){
            ///SFtI
            ...
            event2.data.fd = fd;
            epoll_ctl(efd, EPOLL_CTL_ADD, fd, &event2);///add incoming socket to epoll
            add_client(fd);
          }
        } else{
          while(1){
...
int main(int argc, char **argv) {
    ...
    while(1){
      ...
      if(send(fd, &msg, sizeof(msg), 0) == -1) {
         perror("Failed to send task task");
      }
      jobid++;
    }
  return 0;
