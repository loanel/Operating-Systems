...
void help(){
  printf("Usage for client:\n name mode[1 - unix client, 0 - udp client] IP/UNIX_PATH PORT/nothing");
}
...
void initialize_unix_socket(){
  ...
  /// for some reason bind doesn't work with the unix name, so ill just random something here
  int randint = rand();
  sprintf(bindhelp, "%d", randint);
  bindhelp[9] = '\0';
  strncpy(address.sun_path, bindhelp, PATH_MAX);
  unlink(bindhelp);
  printf("Bind to %s\n", bindhelp);
